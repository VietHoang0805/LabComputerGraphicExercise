...
// Bien toan cuc luu tru toa do cua tam va ban kinh lon, ban kinh nho cua ellipse
...
void mouse(int button, int state, int mouseX, int mouseY) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) {
        if (isFirstClick) {
            ...
            radiusX = abs(dx);
            radiusY = abs(dy);
            bresenhamEllipse(centerX, centerY, radiusX, radiusY);
            glFlush();
            isFirstClick = true;
        }
    }
...
void init() {
    ...
    glColor3f(1.0, 0.0, 0.0); // Set mau ellipse mau do
